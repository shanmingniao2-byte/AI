...
// 火山引擎API密钥相关DOM元素
const volcengineApiKeyInput = document.getElementById('volcengine-api-key-input'); // 火山引擎API密钥输入框
const saveVolcengineApiKeyBtn = document.getElementById('save-volcengine-api-key-btn'); // 保存火山引擎API密钥按钮
...
// 文生图功能相关DOM元素
const t2iPromptInput = document.getElementById('t2i-prompt-input'); // 文生图提示词输入框
const t2iNegativePromptInput = document.getElementById('t2i-negative-prompt-input'); // 文生图反向提示词输入框
const t2iAspectRatioOptions = document.querySelectorAll('.t2i-aspect-option'); // 文生图图像比例选择器
const t2iGenerateBtn = document.getElementById('t2i-generate-btn'); // 文生图生成按钮
const t2iGenerating = document.getElementById('t2i-generating'); // 文生图生成中提示
const t2iResultPlaceholder = document.getElementById('t2i-result-placeholder'); // 文生图结果占位符
const t2iResultImage = document.getElementById('t2i-result-image'); // 文生图结果图像容器
const t2iGeneratedImage = document.getElementById('t2i-generated-image'); // 文生图生成的图像
const t2iResultError = document.getElementById('t2i-result-error'); // 文生图结果错误提示
const t2iSaveImageBtn = document.getElementById('t2i-save-image-btn'); // 文生图保存图像按钮
const t2iCopyPromptBtn = document.getElementById('t2i-copy-prompt-btn'); // 文生图复制提示词按钮
const t2iUsePromptBtn = document.getElementById('t2i-use-prompt-btn'); // 文生图使用翻译结果按钮
const t2iClearPromptBtn = document.getElementById('t2i-clear-prompt-btn'); // 文生图清除提示词按钮
const t2iUseNegativePromptBtn = document.getElementById('t2i-use-negative-prompt-btn'); // 文生图使用反向提示词按钮
const t2iPromptCounter = document.getElementById('t2i-prompt-counter'); // 文生图提示词字符计数器
...
let t2iGeneratedImageData = null; // 存储文生图生成的图像数据
let t2iCurrentTaskId = null; // 存储当前文生图任务ID
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 文生图功能事件监听器
    t2iGenerateBtn.addEventListener('click', generateImage); // 生成图片
    t2iSaveImageBtn.addEventListener('click', downloadImage); // 下载生成的图片
    t2iCopyPromptBtn.addEventListener('click', copyImageResult); // 复制生成的图片URL
    t2iUsePromptBtn.addEventListener('click', useGeneratedImagePrompt); // 使用生成的图片提示词
    
    // 图像比例选择器事件监听器
    t2iAspectRatioOptions.forEach(option => {
        option.addEventListener('click', function() {
            // 移除所有选项的active类
            t2iAspectRatioOptions.forEach(opt => opt.classList.remove('active'));
            // 为当前点击的选项添加active类
    ...
    t2iPromptInput.addEventListener('input', updateT2IPromptCharCount); // 文生图提示词字数统计
    t2iNegativePromptInput.addEventListener('input', updateT2INegativePromptCharCount); // 文生图反向提示词字数统计
...
// 更新文生图提示词字数统计
function updateT2IPromptCharCount() {
    const prompt = t2iPromptInput.value;
    const charCount = prompt.length;
    t2iPromptCharCount.textContent = `${charCount}/2000`;
    
    // 根据字数改变颜色和提示
    if (charCount > 2000) {
        t2iPromptCharCount.style.color = '#ff4757';
        t2iPromptCharCount.setAttribute('title', '提示词超出限制，请缩短提示词');
    } else if (charCount > 1800) {
        t2iPromptCharCount.style.color = '#ff6348';
        t2iPromptCharCount.setAttribute('title', '提示词接近限制，建议缩短');
    } else if (charCount > 1500) {
        t2iPromptCharCount.style.color = '#ffa502';
        t2iPromptCharCount.setAttribute('title', '提示词较长，可能影响生成效果');
    } else if (charCount < 10) {
        t2iPromptCharCount.style.color = '#ff7675';
        t2iPromptCharCount.setAttribute('title', '提示词过短，建议添加更多描述');
    } else {
        t2iPromptCharCount.style.color = '#2ed573';
        t2iPromptCharCount.setAttribute('title', '提示词长度适中');
    }
    
    // 添加字数统计动画效果
    t2iPromptCharCount.style.transform = 'scale(1.1)';
    setTimeout(() => {
        t2iPromptCharCount.style.transform = 'scale(1)';
    }, 100);
}

// 更新文生图反向提示词字数统计
function updateT2INegativePromptCharCount() {
    const prompt = t2iNegativePromptInput.value;
    const charCount = prompt.length;
    t2iNegativePromptCharCount.textContent = `${charCount}/2000`;
    
    // 根据字数改变颜色和提示
    if (charCount > 2000) {
        t2iNegativePromptCharCount.style.color = '#ff4757';
        t2iNegativePromptCharCount.setAttribute('title', '反向提示词超出限制，请缩短');
    } else if (charCount > 1800) {
        t2iNegativePromptCharCount.style.color = '#ff6348';
        t2iNegativePromptCharCount.setAttribute('title', '反向提示词接近限制，建议缩短');
    } else if (charCount > 1500) {
        t2iNegativePromptCharCount.style.color = '#ffa502';
        t2iNegativePromptCharCount.setAttribute('title', '反向提示词较长');
    } else {
        t2iNegativePromptCharCount.style.color = '#2ed573';
        t2iNegativePromptCharCount.setAttribute('title', '反向提示词长度适中');
    }
    
    // 添加字数统计动画效果
    t2iNegativePromptCharCount.style.transform = 'scale(1.1)';
    setTimeout(() => {
        t2iNegativePromptCharCount.style.transform = 'scale(1)';
    }, 100);
}
...
async function generateImage() {
    ...
    const negativePrompt = t2iNegativePromptInput.value.trim();
    
    // 获取选中的图像比例
    let selectedRatio = '1:1'; // 默认比例
    t2iAspectRatioOptions.forEach(option => {
    ...
    try {
        ...
        // 隐藏生成中状态
        t2iGenerating.style.display = 'none';
    }
}
...
function displayGeneratedImage(imageUrl, prompt) {
    // 隐藏占位符和错误信息
    t2iResultPlaceholder.style.display = 'none';
    t2iResultError.style.display = 'none';
    
    // 添加淡入动画效果
    t2iResultImage.style.opacity = '0';
    t2iResultImage.style.display = 'block';
    t2iGeneratedImage.src = imageUrl;
    
    // 图片加载完成后显示
    t2iGeneratedImage.onload = function() {
        // 添加淡入效果
        let opacity = 0;
        const fadeIn = setInterval(() => {
            opacity += 0.1;
            t2iResultImage.style.opacity = opacity;
            if (opacity >= 1) {
        ...
        // 显示操作按钮
        t2iSaveImageBtn.style.display = 'inline-flex';
        t2iCopyPromptBtn.style.display = 'inline-flex';
        t2iUsePromptBtn.style.display = 'inline-flex';
        
        // 添加按钮动画效果
        t2iSaveImageBtn.style.animation = 'slideInUp 0.3s ease-out';
        t2iCopyPromptBtn.style.animation = 'slideInUp 0.3s ease-out 0.1s both';
        t2iUsePromptBtn.style.animation = 'slideInUp 0.3s ease-out 0.2s both';
    };
    
    // 图片加载失败处理
    t2iGeneratedImage.onerror = function() {
        t2iResultImage.style.display = 'none';
        t2iResultError.style.display = 'block';
        t2iResultError.textContent = '图片加载失败，请重试';
    };
}

// 复制生成的图片URL
function copyImageResult() {
    if (!t2iGeneratedImageData || !t2iGeneratedImageData.url) {
        showNotification('没有可复制的图片URL', 'error');
        return;
    }
    
    navigator.clipboard.writeText(t2iGeneratedImageData.url)
        .then(() => {
            showNotification('图片URL已复制到剪贴板', 'success');
            
            // 添加复制成功动画效果
            t2iCopyPromptBtn.style.transform = 'scale(0.95)';
            setTimeout(() => {
                t2iCopyPromptBtn.style.transform = 'scale(1)';
            }, 150);
        })
        .catch(err => {
            console.error('复制失败:', err);
...
function downloadImage() {
    ...
    link.href = t2iGeneratedImageData.url;
...
// 使用生成的图片提示词
function useGeneratedImagePrompt() {
    const prompt = t2iPromptInput.value.trim();
    if (!prompt) {
        showNotification('没有可使用的提示词', 'error');
        return;
    ...
    // 添加成功动画效果
    t2iUsePromptBtn.style.transform = 'scale(0.95)';
    setTimeout(() => {
        t2iUsePromptBtn.style.transform = 'scale(1)';
    }, 150);
    
    showNotification('提示词已填充到翻译输入框', 'success');
}
